...
public class CanalKafkaProducer implements CanalMQProducer {
    ...
    private void send(MQProperties.CanalDestination canalDestination, String topicName, Message message)
        ...
        if (!kafkaProperties.getFlatMessage()) {
            ...
            // 发送扁平数据json
//            logger.error("canalDestination.getDist():"+canalDestination.getDist());
            ...
            if (flatMessages != null) {
                ...
                for (FlatMessage flatMessage : flatMessages) {
                    ...
                    if (StringUtils.isNotBlank(canalDestination.getKeys())) {
                        writeNullFlag = false;
                        for (Field f : FlatMessage.class.getDeclaredFields()) {
                            try {
                                for (String key : canalDestination.getKeys().split(",")) {
                                    if (f.getName().equals(key)) {
                                        f.setAccessible(true);
                                        f.set(flatMessage, null);
                                        break;
                                    }
                                }
                            } catch (Exception e) {
                                logger.error("custom keys error", e);
                            }
                        }
                    }
//                    logger.error("writeNullFlag:" + writeNullFlag);
                    //-------------------------------------------------------------------------
                    ...
                    if (canalDestination.getPartitionHash() != null && !canalDestination.getPartitionHash().isEmpty()) {
                        ...
                        records.add(new ProducerRecord<String, String>(topicName,
                                partition,
                                null,
                                writeNullFlag ? JSON.toJSONString(flatMessage, SerializerFeature.WriteMapNullValue) : JSON.toJSONString(flatMessage)));
                    }

                    // 每条记录需要flush
//                    logger.error("records:"+JSON.toJSONString(records));
